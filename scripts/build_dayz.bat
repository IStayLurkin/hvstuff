...
:: Double-click this from Explorer to build, deploy, and debug.
::
::   BUILD OK → action menu:
::     1  Start driver  (deploys, tails log, watches 90s for crash dump)
::     2  Stop driver
::     3  Tail log      (last 40 lines of dayzdriv.log)
...
:: Stop any running instance before touching the binary.
sc.exe stop dayz >nul 2>&1
sc.exe delete dayz >nul 2>&1
ping -n 2 127.0.0.1 >nul

:: Wipe stale outputs so a partial build never leaves a launchable .sys.
...
(
    ...
    if errorlevel 1 ( echo FAILED: link & goto :fail )

    echo [7/7] signtool
    "%SIGNTOOL%" sign /v /fd sha256 /s PrivateCertStore /n DayZTestCert /t http://timestamp.digicert.com/shield/timestamp "%OUT%"
    if errorlevel 1 ( echo FAILED: signtool & goto :fail )
) 2>&1 | powershell -NoProfile -Command "$input | Tee-Object -FilePath '%BUILDLOG%'"
...
(
    ...
    "%MSVC%\cl.exe" %CLFLAGS% %INCS% /Fo"%OBJ%\Payload.obj" "%ROOT%\tests\payload\Payload.c"
    if errorlevel 1 ( echo FAILED: cl Payload.c & goto :payload_fail )

    echo [payload] link
    "%MSVC%\link.exe" /DRIVER /SUBSYSTEM:NATIVE,10.00 /ENTRY:DriverEntry ^
        /INCREMENTAL:NO /NODEFAULTLIB /RELEASE /DEBUG /PDB:"%PAYLOAD_PDB%" /OUT:"%PAYLOAD_OUT%" ^
        "%OBJ%\Payload.obj" ^
        "%WDKLIB%\km\x64\ntoskrnl.lib" "%WDKLIB%\km\x64\BufferOverflowK.lib"
    if errorlevel 1 ( echo FAILED: payload link & goto :payload_fail )

    echo [payload] signtool
    "%SIGNTOOL%" sign /v /fd sha256 /s PrivateCertStore /n DayZTestCert /t http://timestamp.digicert.com/shield/timestamp "%PAYLOAD_OUT%"
    if errorlevel 1 ( echo FAILED: payload signtool & goto :payload_fail )
) 2>&1 | powershell -NoProfile -Command "$input | Tee-Object -FilePath '%BUILDLOG%' -Append"
goto :payload_done
:payload_fail
echo [payload] payload build failed -- continuing
:payload_done
...
:: Auto-tail the driver log for 5 seconds so you see the boot lines immediately.
...
:: Watch up to 90s for a new crash dump.
