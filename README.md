...
| 2026-05-16 | Immediate BSOD on load (Arg1: `0x7ff...` user-mode address) via KDMapper | `AllocEptTable` passed `hi.QuadPart = 0x7FFFFFFFFFFFF000` to `MmAllocateContiguousMemorySpecifyCache`. This upper-bound constraint caused allocations at low physical addresses whose `MmGetVirtualForPhysical` reverse-mapping resolved to a user-mode VA. The EPT hardware walked that user-mode address as a page table during `EptBuildIdentityMap`, faulting immediately. Fix: `hi.QuadPart` set to `0xFFFFFFFFFFFFFFFF` (no upper bound) so all EPT table pages are allocated from the full physical range with guaranteed kernel-space VAs. Secondary guards added: `EptBuildIdentityMap` validates each physical range from `MmGetPhysicalMemoryRanges` and aborts on impossible PA values; `EptMapPage4KB` validates alignment and 52-bit PA bounds on GPA/HPA before any table walk. |
